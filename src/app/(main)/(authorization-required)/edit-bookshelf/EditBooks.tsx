...
export default function EditBooks({ books, setBooks }: Props) {
  const { id } = useParams()
  ...
  const search = () => {
    ...
    fetch(`https://openlibrary.org/search.json?title=${searchQuery}&limit=10`)
      .then((res) => res.json())
      .then((data) => {
        // we are changing books to this format because books we fetched from existing bookshelf are also in this format
